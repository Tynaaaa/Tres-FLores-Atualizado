...
//TECLAS DE MOVIMENTAÇÃO
	...
	function teclaDw(){
		...
		if(tecla==32){//Espaço / Tiro
			//TIRO
			atira(pjx+17,pjy); //chama a função atira (criada lá embaixo) e atribui um valor para x e y. sempre q se chama uma função q contém argumentos é necessário definir os argumentos junto. Aqui os argumentos, ao inves de colocar o x e y como na hr da criação, foi colocado um valor no lugar deles. Essa função é responsável por criar os tiros. 
		}
	}
...
//TIROS
	...
	function atira(x,y){  //x e y são variáveis q estão sendo usadas como argumentos, ou seja, fornecendo informações para a função. O valor delas são dinidas quando a função é chamada dentro da função teclaDw.
	var t=document.createElement("div"); //a div para o tiro é criada 
	var att1=document.createAttribute("class");
	var att2=document.createAttribute("style");
	att1.value="tiroJog"; //class da div t (tiros) = tiroJog
	att2.value="top:"+y+"px;left:"+x+"px"; // o lugar onde a balá será criada é definido 
	t.setAttributeNode(att1);
	t.setAttributeNode(att2);
	document.body.appendChild(t); //adiciona o tiro a tela (MUDAR PARA A DIVPRINCIPAL)
	}
	
	function controleTiros(){
		var tiros=document.getElementsByClassName("tiroJog"); //coloca todos os elementos que possuem como class "tiroJog", ou seja, todos os tiros (criados na função de cima)
		var tam=tiros.length; //pega a quantidade de elementos guardados em tiros (obs: tem o mesmo nome que a varíavel de tanque verde)
		for(var i=0;i<tam;i++){
			if(tiros[i]){ //confirma se o elemento ainda existe
				var pt=tiros[i].offsetTop; //pega a distância do topo da tela 
				pt-=velT; //dimnui desse tamanho (distância do topo da tela) o valor guardado em velT, isso faz com q a bala se mova 
				tiros[i].style.top=pt+"px"; //a distância é mudada pelo css
				colisaoTiroBomba(tiros[i]); //chama a função de colisão (a cada momento ele vê se colidiu ou n) e caso tenha colidido a bala é apagada
				if(pt<0){
